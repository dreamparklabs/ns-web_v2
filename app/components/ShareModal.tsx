...
export default function ShareModal({ isOpen, onClose, fileId, fileName, isShared = false, shareToken }: ShareModalProps) {
  const { user } = useUser();
  const [currentShareToken, setCurrentShareToken] = useState<string | undefined>(shareToken);
  ...
  const handleRegenerateLink = async () => {
    ...
    setIsGenerating(true);
    try {
      // First unshare, then share again to get a new token
      await unshareFile({ fileId, clerkUserId: user.id });
      const result = await shareFile({ fileId, clerkUserId: user.id });
      setCurrentShareToken(result.shareToken);
    } catch (error) {
  ...
  const handleSocialShare = (platform: string) => {
    ...
    switch (platform) {
      ...
      case 'linkedin':
        ...
        break;
      case 'email':
        socialUrl = `mailto:?subject=${encodeURIComponent(`Shared file: ${fileName}`)}&body=${encodedText}%0A%0A${encodedUrl}`;
        break;
      case 'whatsapp':
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-2xl w-full max-w-md overflow-hidden"
          >
            {/* Header */}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 ${
                    isFileShared ? 'bg-purple-600' : 'bg-gray-200 dark:bg-gray-600'
                  } ${isGenerating ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  <span
                    className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                      isFileShared ? 'translate-x-5' : 'translate-x-0'
                    }`}
                  />
                </button>
              </div>
              ...
              {/* Share Link Section */}
              {isFileShared && currentShareToken && (
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-2">
                      <input
                        ...
                        value={shareUrl}
                        readOnly
                        className="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                      />
                      <button
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      disabled={isGenerating}
                      className="px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      ...
                      >
                        <svg className="w-5 h-5 text-blue-500 mb-1" fill="currentColor" viewBox="0 0 24 24">
                      ...
                      <button
                        ...
                        className="flex flex-col items-center p-3 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                      >
                        <svg className="w-5 h-5 text-green-500 mb-1" fill="currentColor" viewBox="0 0 24 24">
              ...
              {!isFileShared && (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Enable public link sharing to generate a shareable link for this file.
                  </p>
                </div>
              )}
            </div>
